...
- 정보를 이용하는 형태가 각 위치별로 차이가 이을 경우에 이용한다.
...
- ` 수평분할 `: 테이블을 로우 단위로 분리
    - 칼럼은 분리되지 않는다.
    - 모든 데이터가 각 Node별로 분리되어 있는 형태
    - 각 Node에 있는 데이터와 다른 Node에 있는 데이터와 항상 배타적으로 존재하며 한군데 집합시켜도 Primary Key에 의해 중복이 발생하지 않는다.
    - 각 Node 별로 사용하는 로우가 다를때 이용
    - 데이터를 수정할 때는 타 Node에 있는 데이터를 원칙적으로 수정하지 않고 자신의 데이터에 대해서 수정하도록 한다.
    - 각 Node에 존재하는 테이블에 대해서 통합처리를 해야하는 경우는 조인이 발생하여 성능 저하가 예상되므로 통합처리 프로세스가 많은지를 먼저 검토한 이후에 많지 않은 경우에 수평분할을 해야한다.
